...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        self.heads = nn.ModuleList([AttentionHead(d_model, d_model // num_attention_heads, d_model // num_attention_heads, d_model // num_attention_heads) for _ in range(num_attention_heads)])
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        mask = torch.tril(torch.ones((seq_len, seq_len), device=input_ids.device)).unsqueeze(0).expand(batch_size, -1, -1)
...
class TransformerForLanguageModeling(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        hidden_logits = self.transformer_decoder(input_ids)
        logits = self.lm_head(hidden_logits)
        return logits
